...
- **Types**: ``action``, ``abstract action``, ``component``, ``struct``,
  ``buffer``, ``resource``, ``enum``, ``package``, ``monitor``, etc.
- **Constraints**: ``constraint``, ``dynamic constraint``, ``constraint foreach``
- **Exec blocks**: ``exec body``, ``exec pre_solve``, ``exec post_solve``
- **Extensions**: ``extend action``, ``extend component``, ``extend struct``
- **Activity**: ``parallel``, ``schedule``, ``select``, ``replicate``, ``repeat``
...
- **References** — reference count across the workspace. Click to run
  *Find All References*.
- **Subtypes** — number of inheriting/extending types. Click to open
  the type hierarchy.
